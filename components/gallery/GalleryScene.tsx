...
import { artworkPositions, type Project, projects } from "./projects";
		...
		<group>
			{markers.map(([x, z]) => (
				...
				>
					...
					<meshStandardMaterial color="#d0d0d0" transparent opacity={0.4} />
				</mesh>
			))}
		</group>
	);
...
export function GalleryScene({
	...
	return (
		<>
			...
			{/* Back section */}
			<mesh
				position={[ROOM_WIDTH / 2, ROOM_HEIGHT / 2, -5]}
			...
			<mesh
				position={[ROOM_WIDTH / 2, ROOM_HEIGHT / 2, 10]}
				rotation={[0, Math.PI, 0]}
			>
			...
			{/* Stanchion barriers */}
			{/* Left side */}
			<Stanchion start={[-2.8, 0, -11]} end={[-2.8, 0, 13]} postCount={6} />
			{/* Right side */}
			<Stanchion start={[2.8, 0, -8]} end={[2.8, 0, 10]} postCount={5} />
